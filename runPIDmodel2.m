...
% --- ضرایب PID قدیمی (بیرون حلقه) ---
Kp_psi = 15;   Ki_psi = 0.1;   Kd_psi = 0;
Kp_u   = 10;   Ki_u   = 1;     Kd_u   = 0.001;

integral_psi        = 0;  
previous_error_psi  = 0;
integral_u          = 0;  
previous_error_u    = 0;
desiredSpeed        = 1;
...
rng(1);                     % برای تکرارپذیری
    ...
    %% --- LPF setup (1st-order IIR) for angle & speed errors ---
fc_ang = 1.0;   % Hz  (زاویه: 0.8 تا 1.5 تست کن)
fc_spd = 0.8;   % Hz  (سرعت: 0.5 تا 1.2 تست کن)
...
voltage_right_values = [];
time_voltage = [];
speed_values = [];           % ← سرعت واقعی سیستم (ستون 13 استیت)
...
% % (اختیاری) لاگ
% if mod(timeStep,2)==1
%     fprintf('t=%5.2f  F=%.2f  FL=%.2f  FR=%.2f  B=%.2f  BL=%.2f  BR=%.2f (m)\n', ...
%             t_curr, dF, d_fl, d_fr, d_b, d_bl, d_br);
% end
...
% --- 1) نگاشت سرعت مرجع به فاصله‌ی هدف (goalDist) ---
v_min   = 0.2;           % حداقل سرعت
v_max   = 1;           % حداکثر سرعت
goalFar = 8.0;            % دورتریمقیاس‌ (بیشتر از این تقریباً v_max)
goalNear= 1.0;            % نزدیک به هدف → v_min

s = (goalDist - goalNear) / (goalFar - goalNear);
s = max(0, min(1, s));    % کلیپ به [0,1]
desiredSpeed = v_min + (v_max - v_min)*s;

% --- 2) اسکیل ایمنی بر اساس نزدیکی مانع (نرم‌کردن قبل از توقف سخت) ---
safetyBuf   = 0.4;                 % بافر ایمنی ربات/مانع
d_near      = min(dF, minDistMovObs) - safetyBuf;
d_slow      =3;                  % از این فاصله شروع کندشدن
d_stop_soft = 1.2;                  % قبل از stop سخت، سرعت را به v_min نزدیک کن

% نگاشت نرم [d_stop_soft .. d_slow] → [0 .. 1]  (smoothstep)
z = (d_near - d_stop_soft) / (d_slow - d_stop_soft);
z = max(0, min(1, z));
z = z^2*(3 - 2*z);

% اعمال اسکیل ایمنی روی desiredSpeedِ حاصل از goalDist
desiredSpeed = v_min + (desiredSpeed - v_min) * z;
% ---- لاگِ خطای سرعت برای شاخص‌ها (RAW) ----
actualSpeed = state(timeStep,13);                 % سرعت واقعی در همین گام
speed_error_array(end+1) = desiredSpeed - actualSpeed;   % e_v(k) = v_ref - v
...
% <-- دقیقاً این خط را همین‌جا بگذار (سرعت مربوط به همین t_curr)
speed_values(end+1) = state(timeStep,13);   % سرعت فعلی ربات
...
% LPF روی خطای زاویه
if ~lpf_ang_init
    eAng_f = angleError_raw;     % راه‌اندازی نرم در نمونه‌ی اول
    lpf_ang_init = true;
else
    eAng_f = eAng_f + alpha_ang*(angleError_raw - eAng_f);
end
angleError = eAng_f;             % از این به بعد با خطای صاف‌شده کار کن
...
if isAvoidActive
    controlMode = 2;
    % ورودی‌های a109: [turning_angle, FL, F, FR]
...
ruleEnergyAvoid = ruleEnergyAvoid + rfA;
    ...
    controlMode = 1;
    % --- PID زاویه (psi) ---
        integral_psi   = integral_psi + angleError*stepSize_time;
        derivative_psi = (angleError - previous_error_psi)/stepSize_time;
        output_psi     = Kp_psi*angleError + Ki_psi*integral_psi + Kd_psi*derivative_psi;
        previous_error_psi = angleError;

actualSpeed     = state(timeStep,13);                  % v واقعی
speedError_raw  = desiredSpeed - actualSpeed;

% LPF روی خطای سرعت
if ~lpf_spd_init
    eSpd_f = speedError_raw;
    lpf_spd_init = true;
else
    eSpd_f = eSpd_f + alpha_spd*(speedError_raw - eSpd_f);
end
speedError = eSpd_f;

integral_u     = integral_u + speedError*stepSize_time;
derivative_u   = (speedError - previous_error_u)/stepSize_time;

    output_u       = Kp_u*speedError + Ki_u*integral_u + Kd_u*derivative_u;
    previous_error_u = speedError;

        % --- ولتاژ چرخ‌ها ---
        voltage_left   = (output_u + output_psi)/2;
        voltage_right  = (output_u - output_psi)/2;

        % % اشباع
        % voltage_left   = max(-6, min(voltage_left , 6));
        % voltage_right  = max(-6, min(voltage_right, 6));
% % >>> فقط چاپ به کنسول (سرعت و ولتاژها، قبل از هر فازی روی ضرایب)
% fprintf('t=%.2f s | v=%.3f m/s | e_v=%.3f | V_L=%.2f V | V_R=%.2f V\n', ...
%         t_curr, actualSpeed, speedError, voltage_left, voltage_right);
        % لاگ خطا
        angle_error_array(end+1) = abs(angleError);
...
%% --- شاخص‌های خطای سرعت ---
timeSpeed = (0:stepSize_time:(numel(speed_error_array)-1)*stepSize_time);
...
label = 'PID2distogoalm2';  % <-- برای هر کنترلر تغییر بده
...
fprintf('Saved PID2distogoalm2-run pack -> run_%s.mat\n', label);
